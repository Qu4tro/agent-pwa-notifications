...
- 🔌 **Connect anything** — a portable [skill](skills/agent-notifications/SKILL.md), a one-line [MCP](#connect-an-agent) server, an OpenAPI spec for ChatGPT Actions, and drop-in [hooks](examples/claude-code-hooks).
...
`pnpm setup` prints your **worker URL** and how to sign in the first time.
Sign-in is by one-time code: with no email sender configured, read the code
from `pnpm exec wrangler tail`. That first sign-in creates the account and shows
its agent key once. Then:

1. Open the hub on your phone, **Add to Home Screen**, open the app, then
   **Settings, Enable notifications**.
2. Give an agent the MCP snippet or the [skill](skills/agent-notifications/SKILL.md).

Once an agent has the key, `agent-notify-pwa open` mints a one-time sign-in link
with a QR, so no later device needs the email round trip. Re-run
`pnpm setup --rotate` to regenerate all keys. See
[public_docs/deploy.md](public_docs/deploy.md) for the full walkthrough.
...
This drops `skills/agent-notifications/SKILL.md` into your agent so it knows how
to reach your hub; then give it your URL and your account key.
...
curl -X POST "$AGENT_NOTIFY_PWA_URL/api/v1/events" \
  -H "Authorization: Bearer $AGENT_NOTIFY_PWA_KEY" -H "Content-Type: application/json" \
  -d '{"agent":"cli-agent","title":"Build finished","priority":1}'
```

**CLI** — [`agent-notify-pwa`](cli) connects an agent, signs you in on your
phone (QR), and sends updates/questions from scripts:

```bash
npx agent-pwa-notifications login    # save + verify your hub URL and account key
agent-notify-pwa connect             # write ./.mcp.json for your agent
...
- **Two credentials, separate concerns.** The account key (agents post/poll)
  and `APP_SECRET` (signs sessions). They rotate independently. Sessions live in
  KV with a TTL; "log out everywhere" bumps an epoch that invalidates them all.
...
See [`PLAN.md`](PLAN.md) for upstream's design rationale, and
[public_docs/](public_docs) for this fork's deployment and notification docs.
