...
import { Button, KindLabel, Row, RowBody, Time, UnreadDot, type TimelineItem } from './ui'
...
export function TaskLine({ t, unread, divider = true }: { t: TaskSummary; unread?: boolean; divider?: boolean }) {
  return (
    ...
    >
      <Row divider={divider} time={<Time at={t.last_activity} />}>
        <RowBody
          title={
...
export function PendingLine({ t, queryKey }: { t: TaskSummary; queryKey: QueryKey }) {
  ...
  return (
    <div className="border-b border-b-line">
      ...
      <div className="flex flex-wrap items-center gap-2 pr-4 pb-3 pl-[69px]">
        {options.map((o) => (
          <Button
            key={o.label}
            variant="primary"
