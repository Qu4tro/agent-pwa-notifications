...
// one is at least 36px tall, so it stays a comfortable touch target.
...
const BASE =
  'inline-flex min-h-9 items-center justify-center gap-1.5 rounded-ui px-3 text-[14px] leading-none disabled:cursor-not-allowed disabled:opacity-50'
...
// A header action: an icon on its own, with the same 36px touch target.
export function IconButton({
  ...
  return (
    <button
      type="button"
      className={`inline-flex size-9 items-center justify-center rounded-ui text-muted hover:bg-surface hover:text-text ${className}`}
      {...rest}
    />
  )
}
...
export const iconButtonClass =
  'inline-flex size-9 items-center justify-center rounded-ui text-muted hover:bg-surface hover:text-text'
